...
async function main() {
  ...
  for (const property of properties) {
    ...
    for (const lang of LANGUAGES) {
      ...
      // 写入 public/（Vite 构建时自动复制到 dist）
      const publicPath = path.join(OUTPUT_DIR, lang, 'property', `${slug}.html`);
      writeHtmlFile(publicPath, html);
      // 如果 dist 已存在，同步写一份（方便本地测试）
      if (!WRITE_TO_DIST && fs.existsSync(DIST_DIR)) {
        const distPath = path.join(DIST_DIR, lang, 'property', `${slug}.html`);
        writeHtmlFile(distPath, html);
      }
      successCount++;
    }
  }
